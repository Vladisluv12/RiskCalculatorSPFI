...
Калькулятор финансового риска для портфеля внебиржевых процентных и валютных деривативов. Поддерживаются процентные свопы (IRS), валютные форварды (FX forward) и валютные свопы (FX swap, currency swap). Каждый поддерживаемый инструмент оценивается как ежедневный временной ряд чистой приведённой стоимости (NPV), после чего на агрегированном P&L портфеля рассчитываются метрики риска: исторический VaR, параметрический VaR/ES, Component VaR, Incremental VaR и Liquidity-adjusted VaR (LVaR).
...
#### Сверка расчётного ядра с независимым расчётом в Excel

Готовая таблица [`reconciliation.xlsx`](reconciliation.xlsx) в корне репозитория содержит четыре листа: «Методика» с описанием испытания, «Данные» с временным рядом из 250 наблюдений по курсу EUR/RUB (источник `src/data/currency/EURRUB.csv`), «Расчёт Excel» с независимыми формулами `PERCENTILE`, `AVERAGEIF`, `NORMSINV`, `NORMDIST` и «Сводка-сверка» с попарным сопоставлением значений программы (Python) и Excel, расчётом относительного расхождения и автоматическим вердиктом по допуску 0,5%. Сверка проведена по 8 риск-метрикам (исторический и параметрический VaR/ES на уровнях 0,95, 0,975, 0,99); фактическое расхождение — менее $10^{-12}$%. Файл служит подтверждением корректности расчётного ядра для ПМИ (сценарий 15).

### Способ 3. Docker и docker-compose

В репозитории лежит `Dockerfile` (Python 3.11 slim, ставит зависимости и запускает Streamlit на порту 8501) и `docker-compose.yml`, который поднимает три инстанса Streamlit (`streamlit1`, `streamlit2`, `streamlit3`) за nginx на порту 80. Балансировщик использует `ip_hash` (sticky sessions), потому что Streamlit держит сессию в памяти процесса. Каталоги с рыночными данными (`currency`, `curves`, `fixings`, `ois_curves`, `ir`, `sample_portfolios`) монтируются в каждый контейнер в режиме read-only. Каталог `src/data/liquidity/` намеренно не монтируется: данные о спредах ликвидности подгружаются пользователем непосредственно в UI на странице LVaR (загрузка CSV или параметрический ввод модели).
...
В каждой папке репозитория, содержащей код, лежит свой `README.md` с описанием конкретных классов, функций и их роли в общей цепочке.
